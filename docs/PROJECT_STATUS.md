...
- Re-run `pnpm --filter @workspace/api-server run build` in an unrestricted local shell. The current sandbox blocks esbuild while it traverses a parent directory; typecheck, tests, and code generation pass.
