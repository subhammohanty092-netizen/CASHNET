...
export class TronGridProvider implements BlockchainFactProvider {
  ...
  async getTransaction(transactionHash: string) { const response = await this.request(`/v1/transactions/${encodeURIComponent(transactionHash)}`); const raw = response.data?.[0]; return raw ? { status: "SUCCESS" as const, data: tronTransaction(raw) } : { status: "EMPTY" as const, data: null }; }
